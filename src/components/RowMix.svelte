<script>
  ...
  let currentTemplateEnd = templates[0];
  function handleTemplateChange(event) {
    currentTemplate = event.detail;
  }
  function handleTemplateChangeEnd(event) {
    currentTemplateEnd = event.detail;
  }
</script>

<section class="rows">
  <div class="container">
    <h3 class="rows__title">grid-row-start + grid-row-end</h3>
    <div class="flex__group">
      <div class="innre__box">
        <h4 class="form__title">grid-row-start:</h4>
        <RadioGroup
          {currentTemplate}
          {templates}
      ...
      <div class="innre__box">
        <h4 class="form__title">grid-row-end:</h4>
        <RadioGroup
          currentTemplate={currentTemplateEnd}
          templates={templatesEnd}
          on:change={handleTemplateChangeEnd}
        />
      </div>
    </div>
    <div>
      <Playground>
        <div class="parent">
          {#each range(1, 10) as item}
            <div
              class="child"
              style="--start: {currentTemplate}; --end:{currentTemplateEnd}"
            >
              {item}
            </div>
          {/each}
        ...
        <CodeBox>
          ...
          <div class="active__box">
            ...
            <p class="inner__styles">
              ...
              </span><br /><span class="active__line">
                grid-row-end: {currentTemplateEnd};
              </span>
            </p>
          </div>
        </CodeBox>
